...
while (len(jog1) > 10):
   print(f'Nome tem que ter menos de 10 caracteres')
   jog1 = input("Entre com jogador 1: ")
...
pygame.display.set_caption("Primeira Copa Badminton ") #Caso fosse mostrado
...
titl = pygame.font.SysFont("comicsans", 20, False)
ric  = pygame.font.SysFont("comicsans", 15, False)
...
while run:
    ...
    win.blit(titulo, (10*window_width/len(torneio), 0.95*window_height))
    win.blit(dev, (0.92*window_width, 0.97*window_height))
    ...
    if game1 == 1:
        ...
        if gcurrent == 0:
            if saque == 1: #Foco no placar1
                pygame.draw.rect(win, (255,165,0), (0.25*window_width + step - x, window_height/4 + y, width, height))
                cursor1 = font.render(str(score[0][0]) , 1, (0,0,0))
                cursor2 = font.render(str(score[1][0]) , 1, (0,255,0))
        
            if saque == 2: #Foco no placar2
                pygame.draw.rect(win, (0,255,0), (0.25*window_width + step - x, window_height/2 + y, width, height))
                cursor2 = font.render(str(score[1][0]) , 1, (0,0,0))
    
            win.blit(cursor1, (0.25*window_width + step, window_height/4))
            win.blit(cursor2, (0.25*window_width + step, window_height/2))
        else:
            ...
            win.blit(p11, (0.25*window_width + step, window_height/4))
            win.blit(p21, (0.25*window_width + step, window_height/2))
    ...
    if game2 == 1:
        ...
        if gcurrent == 1:
            if saque == 1: #Foco no placar1
                pygame.draw.rect(win, (255,165,0), (0.5*window_width + step - x, window_height/4 + y, width, height))
            ...
            if saque == 2: #Foco no placar2
                pygame.draw.rect(win, (0,255,0), (0.5*window_width + step - x, window_height/2 + y, width, height))
                cursor2 = font.render(str(score[1][1]) , 1, (0,0,0))
    
            win.blit(cursor1, (0.5*window_width + step, window_height/4))
            win.blit(cursor2, (0.5*window_width + step, window_height/2))
        else:
            ...
            win.blit(p12, (0.5*window_width + step, window_height/4))
            win.blit(p22, (0.5*window_width + step, window_height/2))
